...
function kct_redirect_clients_to_portal() {
    ...
    if ( ( in_array('kct_client', $user->roles, true) || in_array('kct_developer', $user->roles, true) ) && is_front_page() ) {
...
function kct_handle_client_back_prompt() {
    ...
    $user = wp_get_current_user();
    if ( ! in_array('kct_client', (array) $user->roles, true) && ! in_array('kct_developer', (array) $user->roles, true) ) {
        return;
    }
    ...
    wp_die(
        ...
        . '<p><a href="' . esc_url($logout_url) . '" style="display:inline-block;background:#1f7ac7;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;font-weight:600;">Log out</a></p>' // Fixed: removed stray '<.
        . '</div>',
        'Access restricted',
        array('response' => 403)
    );
